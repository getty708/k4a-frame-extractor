...
bool process_mkv_offline(const char *input_path,
                         ...
                         k4abt_tracker_configuration_t tracker_config = K4ABT_TRACKER_CONFIG_DEFAULT)
{
    ...
    for (int i = 0; i < (int)g_boneList.size(); i++)
    {
        json_output["bone_list"].push_back(
            { g_jointNames.find(g_boneList[i].first)->second, g_jointNames.find(g_boneList[i].second)->second });
    }
...
{
#ifdef _WIN32
    cout << "Usage: k4abt_offline_processor.exe <input_mkv_file> <output_json_file> [processing_mode] [-model "
